...
Entre as linhas 12 e 16 declaramos cinco novas variáveis e atribuímos a elas o resultado de cinco operações. Note que como \inlineJavaScriptCode{n1} referencia uma String, o operador \inlineJavaScriptCode{+} será tratado como operador de concatenação de Strings ao invés de adição, ou seja, \inlineJavaScriptCode{n2} será convertida para String e concatenada em \inlineJavaScriptCode{n1}! Os outros operadores como são aplicáveis apenas à números, \inlineJavaScriptCode{n1} será convertido implicitamente e a operação será realizada. O resultado disso será visto na saída que será gerada e exibida.
...
\begin{enumerate}
    ...
    \item Delimitadas por aspas duplas (aspas/vírgulas dobradas): \inlineJavaScriptCode{"outra string"};
    \item Delimitadas por símbolos do acento grave (símbolo usado na crase): \inlineJavaScriptCode{`mais uma string`};
\end{enumerate}

Os dois primeiros são análogos, com a diferença que quando se usa aspas simples como delimitador e queremos ter uma aspas simples dentro da String, precisamos escapá-la com contrabarra (barra invertida) e as aspas duplas não precisam. Exemplo \inlineJavaScriptCode{'a\'b"c'} corresponde à \destaque{\texttt{a'b"c}}. Ao passo que ao se usar as aspas duplas como delimitador, temos \inlineJavaScriptCode{"a'b\"c"} correspondendo à \destaque{\texttt{a'b"c}}.
